...
export default function ContactForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await fetch("https://formspree.io/f/xqpkyrno", {
        method: "POST",
        headers: {
    ...
    <section id="contact" className="mx-auto max-w-6xl px-6 py-24">
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        ...
        <form
          ...
          noValidate
          className="panel rounded-2xl p-6 space-y-4"
        >
          {sent && (
          ...
          <button
            type="submit"
            disabled={sending}
            className="w-full flex items-center justify-center gap-2 bg-accent text-bg text-sm font-medium rounded-lg py-2.5 hover:bg-accent-strong transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
          >
            <Send size={15} />
